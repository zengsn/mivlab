	...
	<head>
		...
		<script type="text/javascript">
			function addbt(){
				...
				$("#gsytitle").textbox('setValue','');
				$("#gsymark").textbox('setValue','');
				$("#dlg").dialog('open');
			...
			function updbt(id){
				...
				success:function(data){	
					...
					$("#gsytitle").textbox('setValue',ob.sytitle);
					$("#gsymark").textbox('setValue',ob.symark);
				}
			});
		}
		//保存
		function gltj(){
			...
			var sytitle=$("#gsytitle").textbox('getValue');
			var symark=$("#gsymark").textbox('getValue');
			var id=$("#idi").val();
			var flag=$("#flagi").val();
			if(course_id=="0"||classinfo_id=="0"||terms_id=="0"||sytitle==""||symark==""){
				alert('实验信息不完整。');
				return;
			}
			if("add"==flag){
				$.ajax({
					url:'AddSvlt',
					type:'post',
					data:{"tbname":"syinfo","classinfo_id":classinfo_id,"course_id":course_id,"terms_id":terms_id,"sytitle":sytitle,"symark":symark},
					dataType:'json',
					success:function(data){
						alert(data.msg);
					}
				...
				$.ajax({
					url:'UpdSvlt',
					type:'post',
					data:{"tbname":"syinfo","id":id,"classinfo_id":classinfo_id,"course_id":course_id,"terms_id":terms_id,"sytitle":sytitle,"symark":symark},
					dataType:'json',
					success:function(data){
						alert(data.msg);
					}
		...
		//取消
		function rst(){
			window.location.href="InitSvlt?tbname=syinfo";
		}
		//查询
		function cktj(){
			var sqls="select syinfo.*,termname,classname,cname,tname from courseplan,syinfo,classinfo,course,teacher,terms where classinfo.id=courseplan.classinfo_id and terms_id=terms.id and courseplan.id=courseplan_id and courseplan.teacher_id=teacher.id and  course.id=course_id and teacher.id="+${userid};
			var termsid=$("#ckterm").combobox('getValue');
			if(termsid!="0"){
						...
						<tr>
							<td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#c9c9c9">
								<tr>
									...
									<td height="22" bgcolor="#FFFFFF"><div align="center"><strong><span class="STYLE1">实验内容</span></strong></div></td>
								...
								<c:forEach var="a" items="${alist }">
									<tr>
										...
										<td height="22" bgcolor="#FFFFFF"><div align="center"><span class="STYLE3">${a.symark }</span></div></td>
										...
										<td><table width="100%" border="0" cellspacing="0" cellpadding="0">
											<tr>
												...
												<td><table border="0" align="right" cellpadding="0" cellspacing="0">
													<tr>
														<td width="40"><a href="javascript:void(0);" onclick="sybtdown('PagingSvlt?flag=1','syinfo');return false;"><img src="images/first.gif" width="37" height="15"/></a></td>
														<td width="45"><a href="javascript:void(0);" onclick="syybtdown('PagingSvlt?flag=2','syinfo');return false;"><img src="images/back.gif" width="37" height="15"/></a></td>
														<td width="45"><a href="javascript:void(0);" onclick="xyybtdown('PagingSvlt?flag=3','syinfo');return false;"><img src="images/next.gif" width="37" height="15"/></a></td>
														<td width="40"><a href="javascript:void(0);" onclick="wybtdown('PagingSvlt?flag=4','syinfo');return false;"><img src="images/last.gif" width="37" height="15"/></a></td>
			...
			<td style="width: 100%;" align="center">
				<div id="dlg" class="easyui-dialog" title=" "  style="width:100%;max-width:500px;padding:30px 60px;">
					<form id="ff" method="post">
						...
						<div style="margin-bottom:20px">
							<select id="gcourse_id" class="easyui-combobox" name="gcourse_id" style="width:100%"  data-options="label:'课程:',editable:false,valueField:'id', textField:'text'">
								<option value="0">选择课程</option>								
							</select>
						</div>
						<div style="margin-bottom:20px">
							<select id="gclassinfo_id" class="easyui-combobox"  name="gclassinfo_id" style="width:100%"  data-options="label:'班级:',editable:false,valueField:'id', textField:'text'">
								<option value="0">选择班级</option>
							</select>
						</div>
						<div style="margin-bottom:20px">
							<input id="gsytitle" class="easyui-textbox" name="gsytitle" style="width:100%"  data-options="label:'实验标题:'"/>
						</div>
						<div style="margin-bottom:20px">
							<input id="gsymark" class="easyui-textbox" name="gsymark" style="width:100%;height: 200px;"  data-options="label:'实验内容:',multiline:true"/>
						</div>
					</form>
